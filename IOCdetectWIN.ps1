...
PS> . .\IOCdetectWIN.ps1 -file /folder/name.evtx
...
$IOCPatterns = $IOCRegistry = $IOCNetShare = $IOCMalServiceName = $IOCMalServiceFileName = $ioc_Process = $ioc_WinPS = $ioc_WinDef = $ioc_task = $ioc_NetShare = $ioc_processbase64 = $processFilter = $registryFilter = $auditPolicyFilter = $taskFilter = $WinAvFilter = $winPSFilter = $netShareFilter = $serviceFilter = ''
...
$IOCPatterns = @(
  ...
  'powershell/w 01',                                                               # Used in malicious PowerShell commands
  'powershell/w 01 /ep 0/nop/c',                                                   # Used in malicious PowerShell commands
  '/w 1 /nop',                                                                     # Used in malicious PowerShell commands
  'powershell -noP -sta -w 1 -enc',                                                # Used in malicious PowerShell commands
...
filter MultiSelect-StringIOCPatterns ([string[]]$Patterns) {
  foreach ($Pattern in $Patterns) { # Check the current item against all patterns.
    ...
    if ($found) { # If one of the patterns does not match, continue checking same item.
      $_ #We found a match!
    }
  }
}

filter MultiSelect-StringRegIOC ([string[]]$Patterns){
...
Write-Warning ("Checking for suspicious events/IOCs (Events: *Process* 4688, *Tasks* 4698, *Win Defender* 5007, *Network share accessed 5140*, *Service control manager 7045* ):")
...
if ($scanWinShare) { $ioc_NetShare = Get-WinEvent -FilterHashtable $netShareFilter -ErrorAction SilentlyContinue | Select-Object TimeCreated,@{ Name = 'Owner'; expression = { $_.Properties[1].Value } },@{ Name = 'Share Name'; expression = { $_.Properties[7].Value } },@{ Name = 'Share Path'; expression = { $_.Properties[8].Value } } | MultiSelect-StringNetIOC $IOCNetShare }
