...
$outputGrid = 'True'
$saveToFile = 'False'
$hoursToLookBack = -24 #Include -(followed by hours), -48 = Two days, more time = slower scanning speed
...
$IOCPatterns = ('jndi:ldap:',
  ...
  'base64:',
  '<Hidden>true</Hidden>',#Next 55 strings are common in APTs in schtasks/new process creation on infected machines
  '-WindowStyle Hidden',
  'powershell/w 01',
...
$ioc_winav = Get-WinEvent -FilterHashtable $winavfilter -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Message | MultiSelect-String $IOCPatterns
$ioc_winps = Get-WinEvent -FilterHashtable $winpsfilter -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Message | MultiSelect-String $IOCPatterns
...
if ($outputGrid -eq 'True') {
  ...
  if ($ioc_winav -ne '') { $ioc_winav | Out-GridView -Title 'WinAv IOCs EventID 5007' }
  ...
  if ($ioc_winps -ne '') { $ioc_winps | Sort-Object -Unique | Out-GridView -Title 'Win Powershell IOCs EventID 4104' }
}
...
if ($saveToFile -eq 'True') {
  ...
  if ($ioc_winps -ne '') { $ioc_winps | Sort-Object -Unique | Out-File -Append -FilePath $savedir }
  if ($ioc_winav -ne '') { $ioc_winav | Out-File -Append -FilePath $savedir }
