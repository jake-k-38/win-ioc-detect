...
# Optional Audit Policy Change. Audit event 4719 : "If Success auditing is enabled, an audit entry is generated when an attempted change to user rights assignment policy, audit policy, or trust policy is successful."
# Optional Object access - Audit Registry "A registry value was modified". Audit event 4657 : Once enabled, please set SACL auditing permissions for keys you want to monitor - 
...
# Optional Turn on Audit Net Share access 5140 - Network share object was accessed check for files being remotely accessed
...
$obfuscateCommandSensitivity = 10
$obfuscateChars = @(
  '`',
  '"',
  "'",
...
$IOCPatterns = $IOCRegistry = $IOCNetShare = $IOCMalServiceName = $IOCMalServiceFileName = $ioc_Process = $ioc_SysmonProcess = $ioc_SysmonRegistry = $ioc_WinPS = $ioc_WinDef = $ioc_task = $ioc_NetShare = $ioc_processbase64 = $processFilter = $processSysmonFilter = $registryFilter = $registrySysmonFilter = $auditPolicyFilter = $taskFilter = $WinAvFilter = $winPSFilter = $netShareFilter = $serviceFilter = ''
$ioc_taskbase64 = New-Object System.Collections.ArrayList
$savedir = $saveDirMaster + $datestring + '.txt'
$savedir2 = $saveDirMaster + $datestring + '_base64.txt'
...
try {
  if (-not (Test-Path -Path $file) -or -not (($file -like '*.evtx'))) {
...
$IOCPatterns = @(
  ...
  'cmd.exe /Q /c start',# Call cmd to start script quietly, suspicious
...
filter TaskContent-ConvertBase64String {
  ...
  $extractedBase64 = $_ | Select-String -Pattern $regex -AllMatches | ForEach-Object { $_.Matches } | ForEach-Object { $_.Value }
...
filter MultiSelect-StringIOCPatterns ([string[]]$Patterns) {
  ...
  if ($_.CommandLine -or $_.TaskContent) {
    ...
    foreach ($o in $obfuscateChars){
      ...
      if ($num_obfuscationCommandLine -gt $obfuscateCommandSensitivity -Or $num_obfuscationTaskArguments -gt $obfuscateCommandSensitivity) { $_ } #Sus commandline / Task arguments might be obfuscated
    }
  }
}
...
if ($scanWinProcess) { $ioc_SysmonProcess = Get-WinEvent -FilterHashtable $processSysmonFilter -ErrorAction SilentlyContinue | Select-Object TimeCreated,@{ Name = 'User'; expression = { $_.Properties[12].Value } },@{ Name = 'ParentProcessName'; expression = { $_.Properties[21].Value } },@{ Name = 'NewProcessName'; expression = { $_.Properties[4].Value } },@{ Name = 'CommandLine'; expression = { $_.Properties[10].Value } } | MultiSelect-StringIOCPatterns $IOCPatterns }
...
if ($scanBase64Process) { $ioc_processbase64 = ($ioc_Process | CommandLine-ConvertBase64String) }
if ($scanBase64Tasks) { 
...
if ($saveToFile) {
  ...
  if ($ioc_SysmonProcess -ne '') { $ioc_SysmonProcess | Select-Object * | Out-File -Append -FilePath $savedir }
  if ($ioc_Registry -ne '') { $ioc_Registry | Select-Object * | Out-File -Append -FilePath $savedir }
  if ($ioc_SysmonRegistry -ne '') { $ioc_SysmonRegistry | Select-Object * | Out-File -Append -FilePath $savedir }
  ...
  if ($ioc_WinDef -ne '') { $ioc_WinDef | Select-Object -ExpandProperty Message | Out-File -Append -FilePath $savedir }
  if ($ioc_NetShare -ne '') { $ioc_NetShare | Out-File -Append -FilePath $savedir }
  if ($ioc_SysmomPipe -ne '') { $ioc_SysmomPipe | Out-File -Append -FilePath $savedir }
  if ($ioc_Service -ne '') { $ioc_Service | Out-File -Append -FilePath $savedir }
